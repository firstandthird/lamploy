  ...
  async.auto({
    ...
    writeConfigs: ['buildConfig', (cb, results) => {
      ...
      async.forEachOf(functionList, (funcDir, key, acb) => {
        ...
        let fileContents = fs.readFileSync( path.join(funcDir._tempBundleDir, handler), {encoding: 'utf-8'});
        ...
        fs.writeFile(path.join(funcDir._tempBundleDir, handler), _.join(fileArr, "\n"), (err) => {
          acb(err);
        }); 
      }, (err) => {
        cb(null, true);

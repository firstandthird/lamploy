...
module.exports = function(opts, done) {
  ...
  if(!opts.namespace) {
    opts.namespace = path.basename(opts._cwd);
  }
  ...
  switch(action) {
    case 'list':
      list.list(opts, (err, data) => {
        ...
        _.each(data.Functions, (func) => {
          if( func.FunctionName.indexOf( opts.namespace ) > -1 || opts.listAll === true)  
    ...
    case 'deploy':
      async.auto({
        ...
        bundleFunctions: ['prepareFunctions', (cb, results) => {
        ...
        configureFunctions: ['bundleFunctions', (cb, results) => {
          console.log(colors.yellow('Configuring Functions'));
          configure(results.bundleFunctions, opts, (err, data) => {
            if(err) { return cb(err); }
